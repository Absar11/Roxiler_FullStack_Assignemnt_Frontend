...
const StoreCard = ({ store }) => {
    ...
    const handleRatingClick = (newRating) => {
        if (window.confirm(`Do you want to ${hasRated ? 'modify' : 'submit'} your rating (${newRating} stars) for ${store.name}?`)) {
                ...
                {[1, 2, 3, 4, 5].map(star => (
                    <svg
                        ...
                        className={`w-5 h-5 cursor-pointer ${
                            star <= (ratingHover || ratingValue) ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-300'
                        }`}
    ...
    return (
        <div className="bg-white p-5 shadow-lg rounded-xl border border-gray-200 flex flex-col justify-between h-full hover:shadow-xl transition-shadow">
            <div>
                <h3 className="text-xl font-bold text-indigo-700">{store.name}</h3>
                <p className="text-gray-500 text-sm mb-3 truncate">{store.address}</p>
